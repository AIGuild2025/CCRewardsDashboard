...
CARD_NAMES = [
    "HDFC Regalia Mastercard",
    "HDFC Millennia Mastercard",
    "HDFC MoneyBack Mastercard",
    "HDFC Tata Neu Mastercard",
    "HDFC IndianOil Mastercard"
]
...
def get_transactions(count, include_malformed=False):
    transactions = []
    ...
    if include_malformed:
        malformed_date = start_date + timedelta(days=random.randint(0, 25))
        transactions.append({
            "date": malformed_date.strftime("%d/%m/%Y"),
            "description": "",
            "amount": 0
        })
...
def generate_hdfc_mastercard_pdf(output_path, pdf_number):
    ...
    card_last4 = f"{3000 + pdf_number:04d}"
    ...
    include_malformed = (pdf_number == 3)
    transactions = get_transactions(transaction_counts[pdf_number - 1], include_malformed)
    ...
    for trans in transactions:
        if trans["description"] == "" and trans["amount"] == 0:
            trans_data.append([
                trans["date"],
                "",
                ""
...
def generate_all_hdfc_mastercard_pdfs(output_dir):
    os.makedirs(output_dir, exist_ok=True)
    metadata_list = []

    for i in range(1, 6):
        output_path = os.path.join(output_dir, f"hdfc_mastercard_statement_{i}.pdf")
        metadata = generate_hdfc_mastercard_pdf(output_path, i)
...
if __name__ == "__main__":
    ...
    generate_all_hdfc_mastercard_pdfs(output_dir)
